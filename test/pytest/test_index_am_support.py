...
def test_every_access_method_the_page_claims_builds_and_answers(pgc_conn, expect):
    ...
    # COUNTED, not a flag. The loop below is derived from `ams`, and a premise
    # that reports only "named" bounds nothing a reader or a guard can see. The
    # shell twin counts with `wc -l`; this counts with `len`, and
    # `test_loop_coverage_premise` recognises only the counted form.
    expect.at_least(
        len(ams), 1,
