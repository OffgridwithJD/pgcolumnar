...
#
# AND THE HOLE THIS SUITE CANNOT CLOSE, NAMED RATHER THAN LEFT IMPLICIT. It reads
# ONE bullet. An exclusion written in a NEIGHBOURING bullet -- "spgist is not
# supported", stated on its own -- contradicts the claim and nothing here sees
# it. Widening the window is not the answer: every unrelated edit to the section
# would become a refusal. The refusal message asks the author to split the claim
# and the exclusion into separate sentences, and that instruction followed
# exactly produces the one arrangement this suite is blind to -- so the page says
# where an exclusion belongs instead. Reported by @OffgridwithJD, who noticed that
# the message was steering authors into the blind spot.
#
...
if claim_matches; then
	...
	echo "-- this suite reads NAMES and cannot read prose. A method that does NOT work"
	echo "-- belongs in docs/limitations.md with its evidence, not in a bullet here:"
	echo "-- an exclusion outside the claim bullet is checked against nothing."
fi
