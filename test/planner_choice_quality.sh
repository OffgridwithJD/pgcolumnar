...
# timing noise, and PGC_SKIP_TIMING drops the ratio checks while leaving the
# premises, which is what check_timing is for.
#
...
run_plan() {  # run_plan <settings> <sql> -> "<node> <ms>"
	...
	out=$(env PATH="$PGC_BINDIR:$PATH" psql -h 127.0.0.1 -p "$PGC_PORT" -U postgres \
		-d "$PGC_DB" -Atq -c "SET statement_timeout='120s';" -c "$1" -c '\timing on' -c "$2" 2>&1)
...
choice_vs_best() {  # choice_vs_best <label> <sql>
	...
	if [ "$cms" = TIMEOUT ]; then
		check "[$label] the chosen plan finished at all" "TIMEOUT after 120s" "under the bound"
		return
	fi
